<?php $__env->startSection('content'); ?>
    <div class="book-container">
        ...
        <div class="author-container">
            <p>By <a><?php echo e($user->nickname); ?></a></p>
        </div>
        ...
        <div class="genre-container">
            <a class="genre" genre-id="<?php echo e($book->genre_id); ?>" href="#search"><?php echo e($genre->name); ?></a>
        </div>
    ...
    <div class="book-page-chapter-container">
        <?php $__currentLoopData = $chapters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chapter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="chapter-title-container">
                ...
                $book->title = str_replace(':','', $book->title);
                ?>
                <span><a href="<?php echo e(route('books.chapter', ['name' => $book->title, 'chapterId' => $chapter->id])); ?>" ><?php echo e($chapter->title); ?></a></span>
            </div>
            <div class="chapter-date-container">
                <span><?php echo e(date('d-m-Y', strtotime($chapter->created_at))); ?></span>
            </div>
            </div>
    ...
    <script>
        ...
        $('.rating').on('click', function () {
            ...
            <?php else: ?>
                ...
                if (ratedBookCookie == null) {
                    ...
                    $.ajax({
                        url: '<?php echo e(route('books.rating')); ?>',
                        method: 'POST',
                        data: {
                            clickedStarName,
                            'bookId': '<?php echo e($book->id); ?>'
                        },
                        success: function (resp) {
                            ...
                            var ratedBookArray = {'<?php echo e($book->id); ?>':clickedStarName};
        ...
        if(ratedBookCookie2 != null) {
            if ('<?php echo e($book->id); ?>' in ratedBookCookie2) {
                var keyArray = $('[name=' + "<?php echo e($book->id); ?>" + ']').attr('data-different');
        ...
        function review(){
            ...
            $('#reviewButton').on('click', function () {
                ...
                <?php else: ?>
                    ...
                    $.ajax({
                        url: '<?php echo e(route('book.review.submit')); ?>',
                        method: 'POST',
                        data: {
        ...
        function loadReviews() {
            $.ajax({
                url: '<?php echo e(route('book.reviews.list')); ?>',
                method: 'GET',
                data: {
                    'bookId': '<?php echo e($book->id); ?>'
                },
                    ...
                    if('status' in resp && resp.status == true) {
                        if('reviewList' in resp.data && resp.data.reviewList != undefined || (resp.data.reviewList).length != 0) {
                            (resp.data.reviewList).forEach(function (review) {
                                ...
                                <?php if(\Illuminate\Support\Facades\Auth::guard('members')->check() && \Illuminate\Support\Facades\Auth::guard('members')->id() == $book->user_id): ?>
                                    ...
                                    if('description' in review) {
                                        ...
                                        latestReviewContainerRow.find('.review-title-container .submit-review-btn').on('click', function() {
                                            if(('title' in review) && ('id' in review)) {
                                                ...
                                                $.ajax({
                                                    url:"<?php echo e(route('book.review.edit')); ?>",
                                                    method:'POST',
                                                    data: {
                                                        'userId':'<?php echo e(\Illuminate\Support\Facades\Auth::guard("members")->id()); ?>',
                                                        'id':review.id,
...
<?php echo $__env->make('home.module.template-pages', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\alina\relifeworld\resources\views/book/book.blade.php ENDPATH**/ ?>